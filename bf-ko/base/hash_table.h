...
#else
	rt_mutex_lock(&(t->bucket_locks[n]));
	return 0;
#endif
}
...
#else
	rt_mutex_unlock(&(t->bucket_locks[n]));
	return 0;
#endif
}
...
#else
	return (rt_mutex_trylock(&(t->bucket_locks[n])) == EBUSY);
#endif
}
...
#else
	return (rt_mutex_trylock(&(t->lock)) == EBUSY);
#endif
}
#else
static inline int hash_table_bucket_locked(struct hash_table *t, unsigned int n)
...
#else
		if ((e->key = (unsigned char *)kmalloc(len,GFP_KERNEL)) == NULL)
#endif
			return -1;
...
#else
	     (struct hash_entry *)kmalloc(sizeof(struct hash_entry) * b,GFP_KERNEL)) ==
#endif
	    NULL)
		return -1;
...
#else
	     (struct rt_mutex *) kmalloc(sizeof(struct rt_mutex) * b,GFP_KERNEL)) == NULL)
#endif
		return -1;
...
#else
		__rt_mutex_init(&h->bucket_locks[b], NULL);
#endif
	}
...
#else
	__rt_mutex_init(&h->bucket_locks[0], NULL);
#endif
	if (keycmp)
		h->keycmp = keycmp;
	else
